...
xi <- (recuperaLimites(variavel) +(hi/2))[1:4]
...
poli.freq.acumulada <- ggplot(data = df.poli.freq.acumulada, aes(x = variaveis.valores, y= Fi, group = 1))
poli.freq.acumulada <- poli.freq.acumulada + geom_line() + 
...
poli.freq.acumulada <- poli.freq.acumulada +  
  ...
  theme(axis.title.x = element_text(color="Black", size=30),
        ...
        plot.title = element_text(color="Black",
                                  size=40,
                                  family="Courier"))
rm(Fri.percent, Fi, df.poli.freq.acumulada)
